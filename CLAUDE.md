...
### バックエンド
- **Python 3.12** / **FastAPI**（非同期処理）/ **Pydantic**（型検証）/ **SQLAlchemy**（ORM）
...
| **ホットスポット解消の判定** | **FastAPI 経由のみ** | 6時間ルール・自演防止（同一ユーザー半額）の検証が必要 |
...
PATCH /api/v1/hotspots/{id}/resolve   # 解消判定（6時間ルール・自演防止を検証）
...
- **自演防止**：報告後6時間経過後のみ解消可能（同一ユーザーは解消ポイント半額）
