...
- **2段階で実装する**：
  - **7-A（今）**：ポイント計算ロジックを実装する。写真・GPS は**受け取るが検証は緩め**（任意でも解消可）。まず付与ロジックを通す。
  - **7-B（後）**：**写真必須・GPS必須・報告地点との距離チェック**を有効化する。
- → 7-A で受け取りだけ用意し、7-B で検証を締める。スキーマ/引数は 7-A の時点で 7-B を見越した形にしておくこと。
